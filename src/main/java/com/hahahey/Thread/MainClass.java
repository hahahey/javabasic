...
/**
 *线程周期
 * 1.创建线程对象：指我们继承Thread类或者实现Runnable接口的线程类创建了一个Thread对象(Callable接口)
 * 2.线程就绪：线程对象调用了start方法，此时线程就绪，进入等待队列，可以随时被cpu选中
 * 3.线程运行：当一个处于就绪状态的线程被cpu选中，那么这个线程的代码开始运行，cpu时间片到期或者是调用阻塞类的方法或者是
 *   调用了yield方法join方法等
 * 4.阻塞状态：线程运行中调用了阻塞类的方法，sleep() wait() ,在阻塞状态中，线程是不会运行的，直到阻塞状态结束
 * 5.结束状态：当线程所有的代码都执行完毕，则进入到结束状态
 *
 *
 */
